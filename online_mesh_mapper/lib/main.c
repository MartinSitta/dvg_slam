...
int main(){
    ...
    Chunk_t* chunk = voxel_graph_chunk_hash_table_lookup(graph, 1, 1, 1);
    ...
    Chunk_t* chunk_two = voxel_graph_chunk_hash_table_lookup(graph, 2, -1, 2);
    assert(graph->current_chunk_index == 2);
    assert(chunk_two->x_offset == 0);
    assert(chunk_two->y_offset == -32);
    assert(chunk_two->z_offset == 0);
    assert(chunk_two->current_node_index == 1);

    printf("neighbour connection sanity check:\n");
    ...
    printf("neighbour connection and disconnection check:\n");
    voxel_graph_insert(graph, 1, 1, 2);
    ...
    Chunk_t* chunk_three = voxel_graph_chunk_hash_table_lookup(graph, 32, 0, 0);
