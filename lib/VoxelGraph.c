...
VoxelGraph_t* voxel_graph_init(uint32_t chunk_count){
    ...
    output->chunk_hash_table = malloc(sizeof(AltChunk_t*) * chunk_hash_table_size);
...
AltChunk_t* voxel_graph_chunk_hash_table_lookup(VoxelGraph_t* graph, int64_t x, int64_t y, int64_t z){
    ...
    for(uint32_t i = 0; i < 10; i++){
        ...
        if(graph->chunk_hash_table[double_hash] == NULL){
            ...
            AltChunk_t* chunk = graph->chunk_hash_table[double_hash]; 
...
AltChunk_t* voxel_graph_create_chunk(VoxelGraph_t* graph, int64_t x, int64_t y, int64_t z){
    ...
    AltChunk_t* chunk = malloc(sizeof(AltChunk_t));
    if(chunk == NULL){
        return NULL;
    }
    alt_chunk_init(chunk);
    chunk->x_offset = alt_chunk_build_anchor_coord(x);
    chunk->y_offset = alt_chunk_build_anchor_coord(y);
    chunk->z_offset = alt_chunk_build_anchor_coord(z);
